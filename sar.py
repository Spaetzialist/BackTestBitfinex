 ...
 #   barsdata = pd.read_csv(sys.argv[1])
  #  # Reindex the data: ascending dates are expected in the function
   # barsdata = barsdata.reindex(index=barsdata.index[::-1])
    ...
    closeList, highList, lowList, dateList = utils.fillLists(l)

    barsdata['Date'] = dateList
    barsdata['High'] = highList
    barsdata['Low'] = lowList
    barsdata['Close'] = closeList

    startidx = 0
    endidx = len(barsdata['Date'])
